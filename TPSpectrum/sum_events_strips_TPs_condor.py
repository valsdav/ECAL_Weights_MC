...
for stripid, df in dof.groupby("stripid"):
    ...
    for xtalID in xtals["CMSSWID"]:
        inputfiles.append("root://eos{}.cern.ch/{}/pulses_ID{:.0f}.root".format(
            args.eos, args.inputdir, xtalID))
    outputfile = args.outputdir +"/pulses_stripID{}.root".format(stripid)
    
    if args.fix and outputfile in outputfiles:
        continue

    arguments.append("{} {} {}".format(
        outputfile, PU_string, " ".join(inputfiles)))
...
transfer_input_files    = sum_events_stripDF_TPs.x, run_script.sh
...
./sum_events_stripDF_TPs.x output_temp $@
