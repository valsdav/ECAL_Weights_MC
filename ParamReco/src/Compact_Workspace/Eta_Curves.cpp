...
tuple<double, double> EC_bias(int max_rows, double ts, double EB_w[], double EE_w[], bool normalized_A, bool normalized_t0, bool ideal_weights, string weights_type, string PY, double abs_eta_min, double abs_eta_max) 
{
	//bool test_skip = false; // If want to skip lines to test things 
	int debug_val = 0;
	cout << "Computing Total Error for\n";
	cout << "Abs(Eta)_min = " << abs_eta_min << endl; // minimum absolute values of eta for this loop
	cout << "Abs(Eta)_max = " << abs_eta_max << endl; // maximum abs value of eta to compute bias for this loop
	cout << "Computing Total Error for\n";
	...
	params_ss << "data/XTAL_Params_" << PY << ".txt"; // (cmsswId, A, t0, alpha, beta) values	
	...
	// I think while this is true it performs the action, obtaining the next line .. ?
	   ...
	   if(s >> d1 >> d2 >> d3 >> d4 >> d5){ // Do if on XTAL_params line with desired 's' stream extraction parameters	   
		...
		// Always open same parameters file, but need to choose correct EB/EE weights for online. 
		...
		//while( (getline(inparamFile, param_line)) && (getline(inweightsFile, weights_line)) &&  (leave == false)) { // read EB/EE_DOF line
			...
			//if(ss >> d1_ >> d2_ >> d3_ >> d4_ >> d5_ >> d6_){ // if line has numbers, see if ID's match. 
			  // d5_ = ix, d6_ = iy
				...
				double eta = d5_;
				//cout << "abs_eta_min = " << abs_eta_min << endl;
				//cout << "eta = " << eta << endl;
				//cout << "abs_eta_max = " << abs_eta_max << endl;

				// if eta is out of range, don't extract weights 
				// check if eta is in range
				if ( ( abs_eta_min <= abs(eta) ) && ( abs_eta_max > abs(eta) )  ){
				...
				cout << "w10 = " << w10 << endl;*/
				  ...
				  extra_lines += 1;
				  //if (abs_eta_max == 1.8){debug_val += 1;
				  //if (debug_val == 3) exit(0);}
