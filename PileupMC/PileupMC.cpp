...
TTree* PileupMC::simulatePileup(int id, Pulse* pulse, double stripAmplitude, double signalAmplitudeT,
    ...
    for (int ievt = 0; ievt < nEvents; ievt ++ ){
        ...
        for (int ipul = 0; ipul < pulseLength; ipul++){
            if ((BX0 + ipul) < nBX){
                signal_samples.at(BX0 + ipul) = signalTruth * pulse->sample(ipul);
            }
...
TTree* PileupMC::simulatePileupSignalDistribution(int id, Pulse* pulse, TH1F* TPs, int nxtals, int nEvents, 
    ...
    for (int ievt = 0; ievt < nEvents; ievt ++ ){
        ...
        // Sample the energy of the strip for this event from the TPs histogram
        ET_strip = TPs->GetRandom();
        ET = ET_strip / nxtals;
        signalTruth = ET * cosh(eta);
