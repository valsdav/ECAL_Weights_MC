...
parser.add_argument("-st","--strip", type=str, help="Strip ID", required=False)
...
arguments = []
stripid = int(args.strip)


for i, bx in enumerate(train):
    if bx == "1":
        # this is a collision BX
        inputfile = "root://eos{}.cern.ch/{}/samples_stripID{}_BX{}.root".format(
                        args.eos, args.inputdir,stripid, i)

        for _, row in dfw[dfw.stripid==stripid].iterrows():
            if args.mode ==1:
                ext = "root"
            else:
                ext = "txt"
            outputfile = "{}/bias_stripID{}_BX{}_PU{:.0f}_S{:.0f}.{}".format(
                        args.outputdir, stripid, i, row.PU, row.S, ext)
