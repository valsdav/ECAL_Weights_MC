...
map<string, int> trainStruct = {
    {"8b4e", 8}, {"48b7e", 48}
};

map<string , vector<float>> ETbins {
    { "48b7e", {0.,1.,2.,3.,5.,8.,12.,25.,45.,128.}},
    { "8b4e",  {0.,1.,2.,3.,4.,5.,7.,9.,128.}}
    //{ "8b4e", {0.,0.2,0.4,0.6,0.8,1.,1.5,2.,128.} }
};
...
void analyseBias(RNode rdf, string name, string train, vector<pair<int,int>> eta_rings){
    ...
    int train_nbx = trainStruct[train];
    ...
    map< string, RResultPtr<TH2D>> histos2D;
    // id: (profile: n bins)
...
int main(int argc, char** argv){
    ...
    RDataFrame df_pu50s2   ("EcalTPGAnalysis", basedir+"/TPG_dump_2017_PU50_S2.root");
    RDataFrame df_pu50s30  ("EcalTPGAnalysis", basedir+"/TPG_dump_2017_PU50_S30.root");
    ...
    analyseBias(df_pu50s2,    "pu50s2",    train, eta_intervals);
    analyseBias(df_pu50s30,   "pu50s30",   train, eta_intervals);
